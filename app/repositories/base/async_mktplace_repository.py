...
class AsyncMktplaceRepository(ABC, Generic[T]):
    """
    Interface genérica para operações de repositório com o marketplace.
    Aqui esperamos ter um `seller_id` e um `sku`.
    """
    ...
    @abstractmethod
    async def update_by_sellerid_sku(self, seller_id: str, sku: str, entity: T) -> T:
        """
        Atualiza uma entidade existente no repositório.
        """

    @abstractmethod
    async def patch_by_sellerid_sku(self, seller_id: str, sku: str, patch_entity: dict) -> T:
        """
        Atualiza uma entidade somente com os campos informados no dicionário.
        """

    @abstractmethod
    async def delete_by_sellerid_sku(self, seller_id: str, sku: str) -> bool:
        """
